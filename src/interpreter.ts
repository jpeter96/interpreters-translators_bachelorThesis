    ...
    private evaluateBinaryExpression(expression: BinaryExpression): number {
        ...
        switch (expression.operator) {
            case "+":
                return left + right;
            case "-":
                // LOOP languages often operate on natural numbers (>= 0). 
                // Standard definition usually includes modified subtraction (monus): max(0, x - y)
                // or just regular subtraction allowed to go negative?
                // Given "bachelor thesis" and "LOOP/WHILE/GOTO", it's likely theoretical.
                // I will assume standard javascript subtraction for now but keep monus in mind.
